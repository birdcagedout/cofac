...
class RestaurantDataSource extends DataGridSource {
  final Map<String, Map<String, int>> ticketData;
    ...
    return DataGridRow(
      cells: [
        ...
        for (var staff in staffNameList)
          DataGridCell<int> (
            columnName: staff,
            value: ticketData[storeList[index]]![staff],
          ),
      ],
    );
  });
...
class DataGridPage extends StatefulWidget {
  final Map<String, Map<String, int>> ticketData;
  const DataGridPage({required this.ticketData, super.key});
...
class _DataGridPageState extends State<DataGridPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('${targetDate} 식권 정산'),),
      body: Column(
        children: [
          Expanded(
            child: SfDataGrid(
